...
import tech.ydb.core.grpc.impl.grpc.PreferNearestLoadBalancer.EmptyPicker;
import tech.ydb.core.grpc.impl.grpc.PreferNearestLoadBalancer.ReadyPicker;
import tech.ydb.core.grpc.impl.grpc.PreferNearestLoadBalancer.Ref;
import tech.ydb.core.grpc.impl.grpc.PreferNearestLoadBalancer.StickinessState;
...
import static tech.ydb.core.grpc.impl.grpc.PreferNearestLoadBalancer.STATE_INFO;
 ...
 *
 * based on https://github.com/grpc/grpc-java/blob/v1.20.x/core/src/test/java/io/grpc/util/RoundRobinLoadBalancerTest.java
 */
public class PreferNearestLoadBalancerTest {
    ...
    private final Map<List<EquivalentAddressGroup>, Subchannel> dc1 = Maps.newLinkedHashMap();
    private final Map<List<EquivalentAddressGroup>, Subchannel> dc2 = Maps.newLinkedHashMap();
    private final Map<List<EquivalentAddressGroup>, Subchannel> free = Maps.newLinkedHashMap();
    ...
    private void initServers(String dc, Map<List<EquivalentAddressGroup>, Subchannel> localChannels) {
        for (int i = 0; i < 3; i++) {
            ...
            EquivalentAddressGroup stripped = new EquivalentAddressGroup(addr);
            servers.add(eag);
            Subchannel sc = mock(Subchannel.class);
            when(sc.getAllAddresses()).thenReturn(Arrays.asList(stripped));
            subchannels.put(Arrays.asList(stripped), sc);
            localChannels.put(Arrays.asList(stripped), sc);
        }
    }
    ...
    public void setUp() {
        ...
        when(mockHelper.createSubchannel(any(List.class), any(Attributes.class)))
                .then((InvocationOnMock invocation) -> {
                    ...
                    Subchannel subchannel = subchannels.get((List<EquivalentAddressGroup>) args[0]);
    ...
    public void pickAfterResolved() throws Exception {
        PreferNearestLoadBalancer loadBalancer = new PreferNearestLoadBalancer(mockHelper, null);
        ...
        // Will be created 9 channels to 3 dc
        verify(mockHelper, times(9)).createSubchannel(eagListCaptor.capture(),
    ...
    @Test
    public void pickAfterResolvedOneDC() throws Exception {
        PreferNearestLoadBalancer loadBalancer = new PreferNearestLoadBalancer(mockHelper, "DC1");

        final Subchannel readySubchannel = dc1.values().iterator().next();
        loadBalancer.handleResolvedAddressGroups(servers, affinity);
        loadBalancer.handleSubchannelState(readySubchannel, ConnectivityStateInfo.forNonError(READY));

        // Will be created 3 channels to 1 dc
        verify(mockHelper, times(3)).createSubchannel(eagListCaptor.capture(),
                any(Attributes.class));

        assertThat(eagListCaptor.getAllValues()).containsAllIn(dc1.keySet());
        for (Subchannel subchannel : dc1.values()) {
            verify(subchannel).requestConnection();
            verify(subchannel, never()).shutdown();
        }
    ...
    public void pickAfterResolvedWrongDC() throws Exception {
        PreferNearestLoadBalancer loadBalancer = new PreferNearestLoadBalancer(mockHelper, "DC3");

        final Subchannel readySubchannel = subchannels.values().iterator().next();
        loadBalancer.handleResolvedAddressGroups(servers, affinity);
        loadBalancer.handleSubchannelState(readySubchannel, ConnectivityStateInfo.forNonError(READY));

        // DC2 is invalid and must by ignored, all tests are same as for empty dc
        verify(mockHelper, times(9)).createSubchannel(eagListCaptor.capture(),
                any(Attributes.class));
    ...
    public void pickAfterResolvedUpdatedHosts() throws Exception {
        ...
        doAnswer((Answer<Subchannel>) (InvocationOnMock invocation) -> {
            Object[] args = invocation.getArguments();
            return subchannels2.get((List<EquivalentAddressGroup>)args[0]);
        }).when(mockHelper).createSubchannel(any(List.class), any(Attributes.class));
    ...
    public void pickAfterStateChange() throws Exception {
        ...
        verify(mockHelper, times(9)).createSubchannel(any(List.class), any(Attributes.class));
        verifyNoMoreInteractions(mockHelper);
    }
    ...
    public void noStickinessEnabled_withStickyHeader() {
        PreferNearestLoadBalancer loadBalancer = new PreferNearestLoadBalancer(mockHelper, "DC2");
        ...
        Metadata.Key<String> stickinessKey = Metadata.Key.of("my-sticky-key", Metadata.ASCII_STRING_MARSHALLER);
    ...
    public void stickiness_oneSubchannelShutdown() {
        ...
        List<EquivalentAddressGroup> newServers = servers.stream()
                .filter(eag -> !eag.getAddresses().containsAll(sc2.getAddresses().getAddresses()))
    ...
    public void readyPicker_emptyList() {
        ...
        (new ReadyPicker(Collections.<Subchannel>emptyList(), 0, null)).getClass();
    }
