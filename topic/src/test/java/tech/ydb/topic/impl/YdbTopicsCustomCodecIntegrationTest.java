...
public class YdbTopicsCustomCodecIntegrationTest {
    ...
    /**
     * In this test we write several batches, unregister codec
     * Try to read with errors.
     * Our purpose -> next read with correct codec not loss data. To do so we
     * Create another client, register codec and try to read all the data we write
     * <p>
     * 1. Create client1
     * 2. Create topic TEST_TOPIC1 in client1
     * 3. Create custom codec1
     * 4. Register codec with id = 10113 and codec1
     * 5. Write data with codec 10113 five time
     * 6. Unregister codec
     * 7. Read data with errors
     * 8. Create new client client2
     * 9. Register codec with id = 10113 and codec1
     * 10.Read data without errors five times
     */
    @Test
    public void afterReadFailAndRegisterCodecAgainWeShouldReadAllMessages() throws ExecutionException, InterruptedException, TimeoutException {
        client1 = createClient();
        createTopic(client1, TEST_TOPIC1);
        ...
        TopicClient client2 = createClient();
        client2.registerCodec(10113, codec1);
